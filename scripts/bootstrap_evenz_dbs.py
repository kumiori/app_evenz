...
def merged_event_schema() -> Dict[str, Any]:
    schema = dict(EVENT_SCHEMA)
    schema.update(EVENT_LOG_SCHEMA)
    return schema
...
def seed(repo: EvenzRepo) -> None:
    host = repo.ensure_page_by_title(
        ...
        "Andres / Host",
        {
            "access_key": rich_text("BASE-HOST"),
    ...
    demo_player = repo.ensure_page_by_title(
        ...
        "Demo Helper 🌿",
        {
            "access_key": rich_text("BASE-DEMO"),
            "access_key_hash": rich_text(""),
    ...
    repo.log_event(
        ...
        target_type="system",
        summary="Bootstrapped The Base coordination schema and seed payload.",
...
def verify(repo: EvenzRepo) -> None:
    ...
    print("✅ SUGGESTED_SESSIONS DB verified")
...
def main() -> None:
    ...
    if not repo.is_ready():
        raise RuntimeError("Notion is not configured. Set NOTION_TOKEN and database ids first.")

    repo.update_schema(repo.dbs.players, PLAYER_SCHEMA)
    repo.update_schema(repo.dbs.events, merged_event_schema())
